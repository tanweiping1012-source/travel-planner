...
**④ 查酒店必须先登录，这一步无法跳过。** 携程酒店对匿名访问不显示任何价格——
不是显示不全，是一个数字都没有。所以每次查酒店，它都会先停下来请你扫码登录，
这不是选做步骤。同一个平台的机票查询不需要登录，两者不能类比。
...
5. 参考费用、价格渠道和查询时间——机票、住宿都按这套规则来，住宿总价按晚数
   和登录时看到的房价算出，不是卡片上的起价
6. 可行性状态、硬冲突、风险和调整建议
...
实测发现的：携程酒店对匿名访问**不显示任何价格**，不是显示不全，是一个数字
都没有——请求会直接跳转到登录页。所以酒店查询和小红书走同一套规则：先停，
请你扫码登录，看到价格再继续。这一步不能跳过，也不能拿"机票不用登录"去类推，
同一个网站的两个功能，登录要求完全不同。
...
小红书、OTA 机票、OTA 酒店 **严格串行**——不会同时开两个 Browser Use 阶段。
并行会在标签页焦点、登录接管、失效的元素句柄和页面计时器上互相干扰；酒店和
机票即使同属一个网站，登录状态也不共享，各自的登录交接必须分开处理。
...
**OTA 机票**：在小红书研究和路线骨架完成后才启动；默认查询匿名网页公开价；只有你
明确需要会员价时才请求手动登录。

**OTA 酒店**：与机票不是同一回事——**没有匿名模式**。携程酒店对未登录访问不展示
任何价格，所以每次查酒店都会先停下请你扫码登录，这一步是必须的，不是可选项。

...
高德的地点数据以中国大陆为主，12306 同理。查摩尔曼斯克这类地方时，两个数据
源同时失效，只剩 Browser Use。

这里有个必须知道的细节：**高德对境外查询不返回空，而是返回名称相近的国内
地点**，坐标格式完全正常——查「东京」会得到广西一个真名叫东京的村庄。这个
Skill 会识别并**拒绝**这类结果，而不是拿它去算车程，但代价是境外目的地拿不到
任何已核实的坐标和路线。
...
直接用自然语言说清楚就行。除了出发地、目的地、日期、人数和预算，只有三件事
必须由你回答，因为它们没有安全的默认值：
...
其余的都会先假设再继续，并在方案里写明假设了什么：没说必去景点就当作都可
让路，没说步行上限就按体力强度取值，没说海拔耐受就按可接受处理（真出现高
海拔核心景点时会再问你一次）。

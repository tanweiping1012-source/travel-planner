...
| 小红书说 | 怎么处理 |
|---|---|
| "这三个点一天能串完" | 当作候选路线 → 高德算真实车程 → 可行性检查判断能否成立 |
| "打车过去 20 分钟" | 当作线索 → 高德重新计算 → 以高德为准 |
| "包车 200 一天" | 当作行情参考 → 标注来源与时间，不当作已核实价格 |
| "北门进人少" | 直接采纳为玩法建议 → 无需核实，但标明出处 |

营业时间、票价、车次这类硬事实，一律以官方或高德/12306 为准。这样既用上了
社区攻略的全部信息量，又不会让二手信息变成方案里的"事实"。
...
- **机票价格变化很快。** 铁路票价基本不浮动，机票几小时就可能变，所以每条
  机票记录都带 `checked_at`，并在方案呈现前重新刷新一次。

  当前代码对机票只校验"是否带渠道和时间戳"，还不校验时间戳是否已经过期，
  也不校验时刻与时长是否自洽。这是已知的薄弱环节：呈现前的刷新目前依赖
  工作流约定，而非确定性检查。看到机票价格时，请留意方案中标注的查询时间。
